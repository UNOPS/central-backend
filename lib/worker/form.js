...
const pushDraftToEnketo = ({ Form, enketo, env }, event) =>
  Form.getByActeeIdForUpdate(event.acteeId, undefined, Form.DraftVersion())
    .then((maybeForm) => maybeForm.map((form) => {
      ...
      const path = `${env.internalDomain || env.domain}/v1/test/${form.def.draftToken}/projects/${form.projectId}/forms/${encodeURIComponent(form.xmlFormId)}/draft`;
...
const pushFormToEnketo = ({ Actor, Session, Form, enketo, env }, event) =>
  Form.getByActeeIdForUpdate(event.acteeId)
    .then((maybeForm) => maybeForm.map((form) => {
      ...
      const path = `${env.internalDomain || env.domain}/v1/projects/${form.projectId}`;
